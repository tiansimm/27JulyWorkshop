...
public class ToDoList {
    ...
    public void onAddItem() {
        ...
        // TODO Solution: Check that the input isn't blank, create item, and add to user's list
        if (description != null && !description.trim().isEmpty()) {
            ToDoItem toDoItem = new ToDoItem(description);

            // Call the add method on the user's to-do items list
            user.getToDoItems().add(toDoItem);

            System.out.println("Item added: " + description);
        } else {
            System.out.println("Don't leave the description blank!");
        }
    }
    ...
    public void onMarkAsDone() {
        ...
        if (index >= 0 && index < user.getToDoItems().size()) {
            user.getToDoItems().get(index).setDone(true);
            System.out.println("Changes made! Item marked as done!");
        } else {
    ...
    public void onMarkAsUndone() {
        ...
        int index = itemNumber - 1;
        if (index >= 0 && index < user.getToDoItems().size()) {
            // Option A: If ToDoItem has a setter
            user.getToDoItems().get(index).setDone(false);
            System.out.println("Changes made! Item marked as undone!");
        } else {
            System.out.println("Invalid item number! Please try again!");
        }
    }
    ...
    public void onDeleteItem() {
        ...
        int index = itemNumber - 1;
        if (index >= 0 && index < user.getToDoItems().size()) {
            user.getToDoItems().remove(index);
            System.out.println("Congrats! Item deleted!");
        } else {
            System.out.println("Invalid item number! Please try again.");
        }
    }
